...
function loadPage({ answers = [], generating = false } = {}) {
    ...
    chrome: {
      runtime: {
        sendMessage: async (message) => {
          ...
          return message.type === 'p1.completion' ? reply : { ok: true };
        },
      },
    },
  };
...
const completions = (messages) => messages.filter((message) => message.type === 'p1.completion');

test('existing history is never reported without an observed generation', async () => {
...
test('continuation writes the returned text into the composer and clicks Send', async () => {
  ...
  page.setReply({ continueWith: 'fake result' });
  await page.advance(500, 1);
  page.page.disabled = true;
  page.page.stopVisible = false;
  await page.advance(500, 10);
  assert.deepEqual(page.page.sent, ['fake result']);
  const result = page.messages.find((message) => message.type === 'p1.continuation-result');
