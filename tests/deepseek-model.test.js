...
function load(buttons, { origin = 'https://chat.deepseek.com' } = {}) {
...
test('aria-pressed counts as on; nothing on is the default mode; Send is ignored', () => {
  ...
  assert.equal(idle.sent[0].model.mode, 'Default');
});

test('an unchanged mode is reported once; other sites get nothing', () => {
  const page = load([button('DeepThink', 'selected')]);
  page.tick();
  page.tick();
