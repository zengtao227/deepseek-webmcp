...
function cloneJsonValue(value, state, depth = 0) {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new ToolCallParseError('Tool arguments contain a non-finite number.', 'ARGUMENT_NUMBER');
    return value;
  }
  if (Array.isArray(value)) return value.map((entry) => cloneJsonValue(entry, state, depth + 1));
  ...
  for (const [key, entry] of Object.entries(value)) {
    state.keys += 1;
    if (state.keys > MAX_ARGUMENT_KEYS) throw new ToolCallParseError('Tool arguments contain too many object keys.', 'ARGUMENT_KEYS');
    if (FORBIDDEN_KEYS.has(key)) throw new ToolCallParseError(`Forbidden tool argument key: ${key}`, 'ARGUMENT_KEY');
...
function parseOneBlock(block) {
  if (block.length > MAX_BLOCK_LENGTH) throw new ToolCallParseError('Tool-call block is too large.', 'BLOCK_TOO_LARGE');
  let parsed;
  try {
  ...
  if (Object.keys(parsed).some((key) => !['id', 'name', 'arguments'].includes(key))) {
...
export function parseToolCalls(text) {
  if (typeof text !== 'string') throw new ToolCallParseError('Assistant response must be text.', 'INVALID_RESPONSE');
  const calls = [];
  const ids = new Set();
  let cursor = 0;
  ...
  while (true) {
    ...
    if (close === -1) throw new ToolCallParseError('Tool-call marker is not closed.', 'UNCLOSED_MARKER');
    if (calls.length >= MAX_TOOL_CALLS) throw new ToolCallParseError('Assistant requested too many tools in one response.', 'TOO_MANY_CALLS');

    const call = parseOneBlock(text.slice(blockStart, close).trim());
    if (ids.has(call.id)) throw new ToolCallParseError(`Duplicate tool-call id: ${call.id}`, 'DUPLICATE_ID');
    ids.add(call.id);
    calls.push(call);
